...
<?php
	...
	$M->AddText("Email","Email",false);
	$M->AddPWD("Pwd","密碼",false);
...
function Add(){
	...
	if($YesNo=="true"){
		//檢查Acc是否已經存在	
		$SQL="Select Count(SerialNo) As Num From ".$DBTable." Where Acc ='".$_REQUEST["Acc"]."'";
...
function Modify(){
	...
	if($YesNo=="true"){
		//檢查Acc是否已經存在
		$SQL="Select Count(SerialNo) As Num From ".$DBTable." Where Acc ='".$_REQUEST["Acc"]."' And SerialNo <>".$G[$Level];
		$Rs = mysql_query($SQL,$Conn);
		$Num = 0;
		if($Rs && (mysql_num_rows($Rs)>0)){
			$Row = mysql_fetch_array($Rs);
