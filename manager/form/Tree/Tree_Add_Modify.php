...
<?php
	...
	$M = new Manager();
	$M->AddNum("Sort",$DBFieldName["Sort"],true,4,"","");
	$M->AddText("Tree_ID",$DBFieldName["Tree_ID"],true);
	$M->AddText("Tree_Name",$DBFieldName["Tree_Name"],true);
	$M->AddText("Href_File",$DBFieldName["Href_File"],false);
	$M->AddText("FileName",$DBFieldName["FileName"],false);
	$M->AddSelect2("AdminUse",$DBFieldName["AdminUse"],true,array("Y", "N"),array("Y", "N"),"N");
	//新增用SQL
	$AddFieldsSQL = "";
	$AddValuesSQL = "";
	...
	if($Option=="Add"){
		...
		if($YesNo=="true"){
			// 檢查Tree_ID是否已經存在
			$SQL="Select Count(*) As Num From ".$DBTable." Where Tree_ID = :Tree_ID";
			$Rs = $Conn->prepare($SQL);
			$Rs->bindParam(':Tree_ID', $_REQUEST["Tree_ID"]);
			$Rs->execute();
			$Num = $Rs->fetchColumn();
			if($Num == 0){
				$M->AddHandle();
		...
		if($YesNo=="true"){
			// 檢查Tree_ID是否已經存在
			$SQL="Select Count(*) As Num From ".$DBTable." Where Tree_ID = :Tree_ID And SerialNo <> :SerialNo";
			$Rs = $Conn->prepare($SQL);
			$Rs->bindParam(':Tree_ID', $_REQUEST["Tree_ID"]);
			$Rs->bindParam(":SerialNo", $G[$Level]);
			$Rs->execute();
			...
			<table width="560" cellpadding="0" cellspacing="0" bordercolorlight="#FFFFFF" style="border:solid #A3BFE2 1px;margin-bottom:25px" border="0" bgcolor="#FFFFE1">
				<tr>
					<td bgcolor="#E0EFF8" align="center" height="30" style="font-size:12pt;color:#000000">
                                            <img src="../../images/menu.gif" BORDER="0" align="absmiddle">&nbsp;<?php echo $TopTitle; ?>&nbsp;&nbsp;&nbsp;
					</td>
				</tr>
			</table>
		</td>
