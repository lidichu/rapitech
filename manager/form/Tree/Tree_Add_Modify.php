...
<?php
	...
	$M = new Manager();
	$M->AddNum("Sort","排序",true,4,"","");
	$M->AddText("Tree_ID","功能編號",true);
	$M->AddText("Tree_Name","功能名稱",true);
	$M->AddText("Href_File","檔案位置",false);
	$M->AddText("FileName","檢查名稱",false);
	$AdminUse[0] = "Y";
	$AdminUse[1] = "N";
	$M->AddSelect2("AdminUse","管理者可見",true,$AdminUse,$AdminUse,"N");
...
function Add(){
	...
	if($YesNo=="true"){
		//檢查Tree_ID是否已經存在	
		$SQL="Select Count(SerialNo) As Num From ".$DBTable." Where Tree_ID ='".$_REQUEST["Tree_ID"]."'";
		...
		if($Num == 0){
			...
			$SerialNo = mysql_insert_id();
			$SQL = "Select * From ".$DBTable." Where SerialNo = ".$SerialNo;
			$Rs = mysql_query($SQL,$Conn);
			if($Rs && mysql_num_rows($Rs) > 0){
				$Row = mysql_fetch_array($Rs);
				$SQL = "Insert Into popedom(`Member_ID`,`Tree_ID`) values(1,'".$Row['Tree_ID']."')";
				mysql_query($SQL,$Conn);				
			}
...
function Modify(){
	...
	if($YesNo=="true"){
		//檢查Tree_ID是否已經存在
		$SQL="Select Count(SerialNo) As Num From ".$DBTable." Where Tree_ID ='".$_REQUEST["Tree_ID"]."' And SerialNo <>".$G[$Level];
		$Rs = mysql_query($SQL,$Conn);
		$Num = 0;
		if($Rs && (mysql_num_rows($Rs)>0)){
			$Row = mysql_fetch_array($Rs);
