...
/*	$FilePathArray = array();
	...
	$M->AddSelectG("Size","尺寸",true,"Size","Size","productsize","","Sort, SerialNo desc","");
	$M->AddSelectG("Color","顏色",false,"Color","Color","productcolor","","Sort, SerialNo desc","");
	//$M->AddText("Size","尺寸",false);
	//$M->AddText("Color","顏色",false);
	$M->AddNum("Price","價格",true,4,"","");
	...
	if($Option=="Add"){
		...
		if($YesNo=="true"){
			...
			$SQL = "Insert Into ".$DBTable."(".$AddFieldsSQL.",Status) values(".$AddValuesSQL.",'上架')";
			$Rs = $Conn->prepare($SQL);
