...
<?php
	...
	//取得價格
	$SQL = "select MIN(price) as minprice,MAX(price) as maxprice from `productprice` WHERE G1=:G1 ";
	$PriceParame = array(":G1"=>$G[$Level]);
	$Rs = $Conn->prepare($SQL);
	foreach($PriceParame as $Key => $Value){
		$Rs->bindParam($Key,$PriceParame[$Key]);
	}
	$Rs->execute();
	$Row = $Rs->fetch(PDO::FETCH_ASSOC);
	$Price = ($Row['minprice']!=""&&$Row['maxprice']!="")? "NT$ ".NumHandle3($Row['minprice'])." ~ NT$ ".NumHandle3($Row['maxprice'])."":"";
	if($G[$Level]!=""&&$G[$Level]!="0"){
		$Price.='&nbsp;&nbsp;<a href="javascript:" onclick="cmdSbu1_onclick();">設定價錢</a>';
		$PicOther='&nbsp;&nbsp;<a href="javascript:" onclick="cmdSbu2_onclick();">設定其他圖片</a>';
	}
	
	$M->AddNum("Sort","排序",true,4,"","9999");
	$M->AddSelect2("Status","狀態",true,$StatusItem,$StatusItem,"上架");
	$M->AddText("ProductID","產品編號",false);
	$M->AddText("ProductName","產品名稱",true);
	//$M->AddText("ProductPrice","產品價格",true);
	if($Price!=""){
	$M->AddTitleCustom("產品價格",$Price);
	}
	$M->AddNote2("ProductMaterial","產品材質",false);
	$M->AddNote2("ProductDirections","產品說明",false);
	$M->AddNote2("ProductSizeColor","產品尺寸",false);
	//$M->AddNote2("SizeList","尺寸表",false);
	...
	if($PicOther!=""){
	$M->AddTitleCustom("其他圖片",$PicOther);
	}
	//新增用SQL
	$AddFieldsSQL = "";
	$AddValuesSQL = "";
	...
	if($Option=="Add"){
		...
		if($YesNo=="true"){
			...
			$SQL = "Insert Into ".$DBTable."(".$AddFieldsSQL.",`RecommendSort`,`NewSort`) values(".$AddValuesSQL.",'9999','9999')";
			...
			$Rs->execute();
			$G1 = $Conn->lastInsertId();
			//$G1 = mysql_insert_id();
			ReturnToPage($ModifyFileName,"新增完成",$UrlCode."&option=Modify&G1=".$G1);
		}
...
function cmdSbu1_onclick(){
	...
	for($i=0;$i<=$Level;$i++){if($VarString!=""){$VarString.="&";}$VarString .="P".$i."=".$P[$i];}
	if($VarString!=""){$VarString = "?".$VarString;}
	echo "window.location.href =\"".$MainFileName03_2.$VarString."\"\n";
	?>
}
//圖片按鈕
function cmdSbu2_onclick(){
	<?php
	$VarString = $UrlCode;
	for($i=0;$i<=$Level;$i++){if($VarString!=""){$VarString.="&";}$VarString .="G".$i."=".$G[$i];}
	for($i=0;$i<=$Level;$i++){if($VarString!=""){$VarString.="&";}$VarString .="SF".$i."=".$SF[$i];}
	...
	for($i=0;$i<=$Level;$i++){if($VarString!=""){$VarString.="&";}$VarString .="P".$i."=".$P[$i];}
	if($VarString!=""){$VarString = "?".$VarString;}
	echo "window.location.href =\"".$MainFileName03.$VarString."\"\n";
	?>
}
</script>
</head>
