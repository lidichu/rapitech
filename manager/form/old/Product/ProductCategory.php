				...
				$ErrorMsg = "";
					for($i=1;$i<=$RowCount;$i++){
						...
						if($tmpCheckBox!=""){			
							...
							$SQL = "Select * From ".$DatabaseName02." Where C0 = :SerialNo";
							$Rs = $Conn->prepare($SQL);
							$Rs->bindParam(":SerialNo",$_REQUEST["SERIALNO".$i]);
							$Rs->execute();
							while($Row3 = $Rs->fetch(PDO::FETCH_ASSOC)){
								if($Row3["PICHidden"]!=""){
									foreach($UploadPic as $Key => $Value){
										$DelFileName = FileHandle::Delete($Value["Path"],$Row3["PICHidden"]);	
									}
								}
							}
							while($Row = $Rs->fetch(PDO::FETCH_ASSOC)){
								$C0=$Row["SerialNo"];
								$SQL = "Select * From ".$DatabaseName03." Where P1 = :SerialNo";
								$Rs2 = $Conn->prepare($SQL);
								$Rs2->bindParam(":SerialNo",$C0);
								$Rs2->execute();
								...
								$SQL = "delete from ".$DatabaseName03." where P1 = :SerialNo";
								$Rs = $Conn->prepare($SQL);
								$Rs->bindParam(":SerialNo", $C0);
								$Rs->execute();
							}
							$SQL = "delete from ".$DatabaseName02." where C0 = :SerialNo";
							$Rs = $Conn->prepare($SQL);
							$Rs->bindParam(":SerialNo", $_REQUEST["SERIALNO".$i]);
							$Rs->execute();
                    ...
                    <td bgcolor="#A3BFE2">
                        <table cellspacing="1" cellpadding="2" width="100%" border="0">
	                        <tr style="font-size:13;color:#E0EFF8" bgcolor="#005783" align="center">
								...
								<td nowrap ><a class="Title sortlink" href="<?php echo GetSCRIPTNAME(); ?>?SF<?php echo $Level; ?>=Title DESC"><font color="#FFFFFF" style="font-size:13px;">類別名稱</font></a></td>
								...
								<td nowrap width="80" ><font color="#FFFFFF" style="font-size:13px;">產品列表</font></td>
							</tr>
                        <?php
