...
<?php
	...
	//參數設定
		...
		$TableTitle = $Title03;
		//要查詢的欄位
		$SQLFields="SerialNo,G1PIC,PICHidden,Sort";
		//預設排序方式
		...
		$SQL = "Select * From ".$DBTable." Where G1 = :G1";
		$Rs = $Conn->prepare($SQL);
		...
		$SQL = "Select * From ".$DBTable." Where G1 = :G1 and G1PIC = '否' order by sort, SerialNo desc ";
		$Rs = $Conn->prepare($SQL);
		$Rs->bindParam(":G1",$G[$Level-1]);
		$Rs->execute();
		$Rows = $Rs->fetchAll();
		...
		if($RowCount!=0&&$RowCount==$RowCount2){
			$SQL = "Update ".$DBTable." Set G1PIC = '是' Where SerialNo= :SerialNo";
			$Rs = $Conn->prepare($SQL);
			$Rs->bindParam(":SerialNo",$Rows[0]['SerialNo']);
			$Rs->execute();
		}
                    ...
                    <td bgcolor="#A3BFE2">
                        <table cellspacing="1" cellpadding="2" width="100%" border="0">
	                        <tr style="font-size:13;color:#E0EFF8" bgcolor="#005783" align="center">
								...
								<td nowrap width="80"><font color="#FFFFFF" style="font-size:13px;">在列表時顯示的圖片</font></td>
								<td nowrap width="220"><font color="#FFFFFF" style="font-size:13px;">產品圖</font></td>
