...
<?php
	...
	//參數設定
		...
		$TableTitle = $Title01;
		//要查詢的欄位
		$SQLFields="SerialNo,IndexShow,PostDate,Title,Sort,Status";
		//預設排序方式
			...
			case "Del":
				...
				for($i=1;$i<=$RowCount;$i++){
					...
					if($tmpCheckBox!=""){
						...
						//刪除相關連結						
						$SQL = "delete from ".$DatabaseName02_1." where G0=:SerialNo";
						$Rs = $Conn->prepare($SQL);
						$Rs->bindParam(":SerialNo", $SerialNo);
						$Rs->execute();
						//刪除檔案及相關檔案
						$SQL = "Select * From ".$DatabaseName02_2." Where G0 = :SerialNo";
						$Rs = $Conn->prepare($SQL);
						...
						$SQL = "delete from ".$DatabaseName02_2." where G0=:SerialNo";
						$Rs = $Conn->prepare($SQL);
						$Rs->bindParam(":SerialNo", $SerialNo);
						$Rs->execute();
						//刪除資料
                    ...
                    <td bgcolor="#A3BFE2">
                        <table cellspacing="1" cellpadding="2" width="100%" border="0">
	                        <tr style="font-size:13;color:#E0EFF8" bgcolor="#005783" align="center">
								...
								<td nowrap ><a class="Title sortlink" href="<?php echo GetSCRIPTNAME(); ?>?SF<?php echo $Level; ?>=Title DESC"><font color="#FFFFFF" style="font-size:13px;">標題</font></a></td>
