...
<?php
	...
	$M = new Manager();
	$M->AddText("Acc","帳號",true);
	$M->AddText("Name","姓名",true);
	$M->AddEMail("Email","Email",false);
	$M->AddMD5PWD("Pwd","密碼",true);
	//新增用SQL
	$AddFieldsSQL = "";
	$AddValuesSQL = "";
	...
	if($Option=="Add"){
		...
		if($YesNo=="true"){
			//檢查Acc是否已經存在	
			$SQL="Select Count(*) As Num From ".$DBTable." Where Acc = :Acc";
			$Rs = $Conn->prepare($SQL);
			$Rs->bindParam(':Acc', $_REQUEST["Acc"]);
			$Rs->execute();
			$Num = $Rs->fetchColumn();
			if($Num == 0){
				$M->AddHandle();
		...
		if($YesNo=="true"){
			//檢查Acc是否已經存在
			$SQL="Select Count(*) As Num From ".$DBTable." Where Acc = :Acc And SerialNo <> :SerialNo";
			$Rs = $Conn->prepare($SQL);
			$Rs->bindParam(':Acc', $_REQUEST["Acc"]);
			$Rs->bindParam(":SerialNo", $G[$Level]);
			$Rs->execute();
			...
			if($Num == 0){	
				...
				$Rs->bindParam(":SerialNo", $G[$Level]);
				$Rs->execute();
				if(strval($BeforeID) == strval($id)){  
					$SQL = "Select * From ".$DBTable." Where SerialNo = :SerialNo";
					$Rs = $Conn->prepare($SQL);
					$Rs->bindParam(":SerialNo", $G[$Level]);
					$Rs->execute();
					$Row = $Rs->fetch(PDO::FETCH_ASSOC);
					setcookie("id", $Row["Acc"], time()+60*60*24, "/", "", 0);
					setcookie("pw", $Row["Pwd"], time()+60*60*24, "/", "", 0);
				}
