			...
			<table style="margin-bottom:20px;" width="600" border="1" cellspacing="0" bordercolorlight="#666666" bordercolordark="#FFFFFF">
				<tr> 
					<td>
						...
						<?php 
							//查詢是否有回覆
							$SQLReply = "Select * From ". $DatabaseName02 . " Where G0 = :G0";
							$Rs = $Conn->prepare($SQLReply);
							$Rs->bindParam(":G0", $Row["SerialNo"]);
							$Rs->execute();
							$Row = $Rs->fetch(PDO::FETCH_ASSOC);
							if($Row){
								$M2->ReadShow();
