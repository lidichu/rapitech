...
<?php
	...
	//參數設定
		...
		$TableTitle = $Title01;
		//要查詢的欄位
		$SQLFields="SerialNo,date_format(`PostDate`,'%Y-%m-%d') As PostDate,`Subject`,`Name`,Tel,`EMail`,`Status` As ContactStatus,'ContactReply.php' as replyemail";
		//預設排序方式
		...
		//可查詢欄位
		$QueryField["Subject"] = $DBFieldName["Subject"];
		$QueryField["Name"] = $DBFieldName["Name"];
		$QueryField["Tel"] = $DBFieldName["Tel"];
		$QueryField["Mobile"] = $DBFieldName["Mobile"];
		$QueryField["EMail"] = $DBFieldName["EMail"];
		$QueryField["Address"] = $DBFieldName["Address"];
		//可查詢列表
		$QueryList=Array();
		//資料庫額外欄外
		$QueryFiled=Array();
		...
		switch ($Option) {
			...
			case "SortUp":
				...
				break;
			case "Del":
				$TitleField = "Subject";	// 不能刪除時紀錄的標題欄位名稱
				$ErrorMsg = "";
				//檢查是否有需要刪除檔案
				$FileField = array();
					...
					if($tmpCheckBox!=""){
						...
						if($CanDelete){
							...
							if(count($FileField) > 0){
								$SQL = "Select * From ".$DBTable." Where SerialNo = ".$_REQUEST["SERIALNO".$i];
								$Rs = $Conn->prepare($SQL);
							...
							//刪除聯絡我們回覆資料
							$SQL="Delete From " . $DatabaseName02 . " Where G0= :G0";
							$Rs = $Conn->prepare($SQL);
							...
							$SQL = "Select ".$TitleField." From ".$DBTable." Where SerialNo = :SerialNo";
							$Rs = $Conn->prepare($SQL);
							$Rs->bindParam(":SerialNo",$_REQUEST["SERIALNO".$i]);
							$Rs->execute();
							$Row = $Rs->fetch(PDO::FETCH_ASSOC);
							if($ErrorMsg != ""){ $ErrorMsg .= "\\n"; }
							$ErrorMsg .= $Row[$TitleField];
						}
					}
				}
				if($ErrorMsg==""){
		...
		$Tree_SQL = "Select Count(*) As Num From ".$DBTable_S.$Query." and Status = '未處理'";
		$Tree_Rs2 = $Conn->prepare($Tree_SQL);
		$Tree_Rs2->execute();
		$Tree_Row2 = $Tree_Rs2->fetch(PDO::FETCH_ASSOC);
		$Tree_AmountC = intval($Tree_Row2["Num"]);
?>
...
var TAC = "<?php echo ($Tree_AmountC); ?>";
$(function(){
	if (window.parent.frames["menu"] != undefined){
		$(window.parent.frames["menu"].document).find("#Menu_ContactManager").html("("+(parseInt(TAC))+")");
		$(window.parent.frames["menu"].document).find("#Tree_OContact").html("("+(parseInt(TAC))+")");
	}
});
</script>
                    ...
                    <td bgcolor="#A3BFE2">
                        <table cellspacing="1" cellpadding="2" width="100%" border="0">
	                        <tr style="font-size:13;color:#E0EFF8" bgcolor="#005783" align="center">
		                        ...
		                        <td nowrap width="100" style="font-size:12px;"><a class="Title sortlink" href="<?php echo GetSCRIPTNAME(); ?>?SF<?php echo $Level; ?>=PostDate DESC"><font color="#FFFFFF" style="font-size:13px;"><?php echo($DBFieldName["PostDate"]); ?></font></a></td>
								<td nowrap style="font-size:12px;"><a class="Title sortlink" href="<?php echo GetSCRIPTNAME(); ?>?SF<?php echo $Level; ?>=Subject"><font color="#FFFFFF" style="font-size:13px;"><?php echo($DBFieldName["Subject"]); ?></font></a></td>
								<td nowrap style="font-size:12px;"><a class="Title sortlink" href="<?php echo GetSCRIPTNAME(); ?>?SF<?php echo $Level; ?>=Name"><font color="#FFFFFF" style="font-size:13px;"><?php echo($DBFieldName["Name"]); ?></font></a></td>
								<td nowrap style="font-size:12px;"><a class="Title sortlink" href="<?php echo GetSCRIPTNAME(); ?>?SF<?php echo $Level; ?>=Tel"><font color="#FFFFFF" style="font-size:13px;"><?php echo($DBFieldName["Tel"]); ?></font></a></td>
								<td nowrap style="font-size:12px;"><a class="Title sortlink" href="<?php echo GetSCRIPTNAME(); ?>?SF<?php echo $Level; ?>=EMail"><font color="#FFFFFF" style="font-size:13px;"><?php echo($DBFieldName["EMail"]); ?></font></a></td>
