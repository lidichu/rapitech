...
<?php
	...
	//參數設定
		...
		$TableTitle = $Title06;
		//要查詢的欄位
		$SQLFields="OL.SerialNo,OL.OrderNumber,date_format(OL.OrderDate,'%Y-%m-%d'),OL.TotalPrice,OL.MemberName,OL.MemberID,OL.ShipDate,OL.BuyWay";
		//預設排序方式
		...
		//PageCount處理
		$PageCount = 0;
		$Query.=" and OD.IsNew='是' and OD.ManagerHandle ='未處理' and OD.PayHandle ='退貨申請' and OL.SerialNo = OD.G0";
		...
		$Tree_SQL = "Select Count(*) As Num From order_list as OL, orderrecord as OD Where true and OD.IsNew='是' and OD.ManagerHandle ='未處理' and OD.PayHandle ='退貨申請' and OL.SerialNo = OD.G0";
		$Tree_Rs2 = $Conn->prepare($Tree_SQL);
		$Tree_Rs2->execute();
		$Tree_Row2 = $Tree_Rs2->fetch(PDO::FETCH_ASSOC);
		$Tree_Amount = intval($Tree_Row2["Num"]);
?>
...
var TAmount = "<?php echo ($Tree_Amount); ?>";
$(function(){
	if (window.parent.frames["menu"] != undefined){
		$(window.parent.frames["menu"].document).find("#Menu_ReturnManager").html("("+(parseInt(TAmount))+")");
		$(window.parent.frames["menu"].document).find("#Tree_RUnhandle").html("("+(parseInt(TAmount))+")");
	}
});
            ...
            <table width="100%" border="1" cellspacing="0" bordercolorlight="#666666" bordercolordark="#FFFFFF">
                <tr>
                    ...
                    <td width="5%" align="right" bgcolor="#EEEEEE">
						<input type="button" value="退換貨完成" id="cmdV" name="cmdV" onClick="cmdSubmit_onclick('<?php echo GetSCRIPTNAME(); ?>');" >
						<?php	if($AdminView){	?>
