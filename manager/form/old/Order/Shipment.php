...
<?php
	...
	//參數設定
		...
		$TableTitle = $Title02;
		//要查詢的欄位
		$SQLFields="OL.SerialNo,OL.OrderNumber,date_format(OL.OrderDate,'%Y-%m-%d'),OL.PayMethod,OL.TotalPrice,OL.MemberName,OL.MemberID,OL.BuyWay";
		//預設排序方式
			...
			case "OrderShip":
				...
				for($i=1;$i<=$RowCount;$i++){
					...
					if($tmpCheckBox!=""){
						...
						//修改訂單出貨時間
						$SQL = "Update order_list Set ShipDate = :ShipDate Where SerialNo = :SerialNo";
						...
						$A="'".$SN."','".$Temp_Order_List["MemberID"]."','".Date('Y-m-d H:i:s')."','已付款','已出貨','是'";
		...
		//PageCount處理
		$PageCount = 0;
		$Query.=" and OD.IsNew='是' and OD.ManagerHandle ='訂單確認' and OL.SerialNo = OD.G0";
...
function cmdSubmit_onclick(){
	if($(".CheckBoxShow:checked").length<=0){
		...
		var msg="是否確定出貨？";
		if(confirm(msg)){
			$("#form1").attr("action","<?php echo GetSCRIPTNAME(); ?>?option=OrderShip");
			$("#form1").submit();
		}
	}	
}
            ...
            <table width="100%" border="1" cellspacing="0" bordercolorlight="#666666" bordercolordark="#FFFFFF">
                <tr>
                    ...
                    <td width="5%" align="right" bgcolor="#EEEEEE">
						...
						<input type="button" value="出貨" id="cmdShip" name="cmdShip" onClick="cmdSubmit_onclick('<?php echo GetSCRIPTNAME(); ?>');" >
