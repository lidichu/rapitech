...
<?php
	...
	if($Option=="Add"){
		...
		$Temp_Order_Record=GetTable("orderrecord","*",$Param,"Order by SerialNo DESC Limit 1",1);
		...
		$A="'".$G[0]."','".$Temp_Order_List["MemberID"]."','".Date('Y-m-d H:i:s')."','".$Temp_Order_Record["PayHandle"]."','交易完成','是'";
		$SQL = "Insert Into orderrecord(".$Q.") values(".$A.")";
		$Rs = $Conn->prepare($SQL);
		$Rs->execute();
		ReturnToPage($MainFileName,"執行完畢","");
	}elseif($Option=="OrderInvalid"){
		$SN=$G[0];
		$Param=array(
			"SerialNo"	=> $SN
		);
...
function cmdSubmit_onclick(){
	if($(".CheckBoxShow:checked").length<=0){
		...
		var msg="是否確定交易完成？";
		if(confirm(msg)){
			$("#form1").attr("action","<?php echo GetSCRIPTNAME(); ?>?option=OrderValid&G0=<?php  echo $G[0] ?>");
			$("#form1").submit();
		}
	}	
}
//交易取消
function cmdV_onclick(){
	if($(".CheckBoxShow:checked").length<=0){
		alert("未選擇訂單");
	}else{
		var msg="是否確定交易取消？";
		if(confirm(msg)){
			$("#form1").attr("action","<?php echo GetSCRIPTNAME(); ?>?option=OrderInvalid&G0=<?php  echo $G[0] ?>");
			$("#form1").submit();
		}
	}	
}
...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	...
	<tr>
		<td align="center">
			...
			<!--
			<input type="button" value="交易完成" id="cmdF" name="cmdF" onClick="cmdSubmit_onclick();">
			<input type="button" value="交易失敗" id="cmdC" name="cmdC" onclick="cmdV_onclick();">
			-->
