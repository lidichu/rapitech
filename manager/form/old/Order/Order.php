...
<?php
	...
	//參數設定
		...
		$TableTitle = $Title01;
		//要查詢的欄位
		$SQLFields="OL.SerialNo,OL.OrderNumber,date_format(OL.OrderDate,'%Y-%m-%d'),OL.ReplyAccount,OL.PayMethod,OL.TotalPrice,OL.MemberName,OL.MemberID,OL.BuyWay";
		//預設排序方式
			...
			case "Del":
				...
				for($i=1;$i<=$RowCount;$i++){
					...
					if($tmpCheckBox!=""){
						...
						//刪除資料-訂單明細
						$SQL = "delete from order_detail where G0 = :G0";
						$Rs = $Conn->prepare($SQL);
						$Rs->bindParam(":G0",$_REQUEST["SERIALNO".$i]);
						$Rs->execute();
						
						//刪除資料-退換貨
						$SQL = "delete from orderreturn where G0 = :G0";
						$Rs = $Conn->prepare($SQL);
						$Rs->bindParam(":G0",$_REQUEST["SERIALNO".$i]);
						$Rs->execute();
						
						//刪除資料-訂單紀錄
						$SQL = "delete from orderrecord where G0 = :G0";
						$Rs = $Conn->prepare($SQL);
						$Rs->bindParam(":G0",$_REQUEST["SERIALNO".$i]);
						$Rs->execute();
						
						//刪除資料-訂單問答
						$SQL = "delete from order_qa where G0 = :G0";
						$Rs = $Conn->prepare($SQL);
						$Rs->bindParam(":G0",$_REQUEST["SERIALNO".$i]);
						$Rs->execute();
			...
			case "AgreeOrder":
				...
				for($i=1;$i<=$RowCount;$i++){
					...
					if($tmpCheckBox!=""){
						...
						$A="'".$SN."','".$Temp_Order_List["MemberID"]."','".Date('Y-m-d H:i:s')."','已付款','訂單確認','是'";
		...
		//PageCount處理
		$PageCount = 0;
		$Query.=" and OD.IsNew='是' and OD.ManagerHandle ='未處理'";
		$Query.=" and OD.PayHandle not in('訂單取消','退貨申請') and OL.SerialNo = OD.G0";
		...
		$Tree_SQL = "Select Count(*) As Num From order_list as OL, orderrecord as OD Where true and OD.IsNew='是' and OD.ManagerHandle ='未處理' and OD.PayHandle not in('訂單取消','退貨申請') and OL.SerialNo = OD.G0";
		$Tree_Rs2 = $Conn->prepare($Tree_SQL);
		$Tree_Rs2->execute();
		$Tree_Row2 = $Tree_Rs2->fetch(PDO::FETCH_ASSOC);
		$Tree_AmountOrder = intval($Tree_Row2["Num"]);
		
		$Tree_SQL = "Select Count(*) As Num From order_list as OL, orderrecord as OD Where true and OD.IsNew='是' and OD.ManagerHandle ='訂單確認' and OL.SerialNo = OD.G0";
		$Tree_Rs2 = $Conn->prepare($Tree_SQL);
		$Tree_Rs2->execute();
		$Tree_Row2 = $Tree_Rs2->fetch(PDO::FETCH_ASSOC);
		$Tree_AmountShipment = intval($Tree_Row2["Num"]);
		
		$Tree_SQL = "Select Count(*) As Num From order_list as OL, orderrecord as OD Where true and OD.IsNew='是' and OD.ManagerHandle ='已出貨' and OL.SerialNo = OD.G0";
		$Tree_Rs2 = $Conn->prepare($Tree_SQL);
		$Tree_Rs2->execute();
		$Tree_Row2 = $Tree_Rs2->fetch(PDO::FETCH_ASSOC);
		$Tree_AmountFinished = intval($Tree_Row2["Num"]);
		
		$Tree_ToTalAmount = $Tree_AmountOrder+$Tree_AmountShipment+$Tree_AmountFinished;
?>
...
var TAOrder = "<?php echo ($Tree_AmountOrder); ?>";
var TASrder = "<?php echo ($Tree_AmountShipment); ?>";
var TAFrder = "<?php echo ($Tree_AmountFinished); ?>";
var TTOrder = "<?php echo ($Tree_ToTalAmount); ?>";
$(function(){
	if (window.parent.frames["menu"] != undefined){
		$(window.parent.frames["menu"].document).find("#Menu_OrderManager").html("("+(parseInt(TTOrder))+")");
		$(window.parent.frames["menu"].document).find("#Tree_OAOrder").html("("+(parseInt(TAOrder))+")");
		$(window.parent.frames["menu"].document).find("#Tree_OShipment").html("("+(parseInt(TASrder))+")");
		$(window.parent.frames["menu"].document).find("#Tree_OFinished").html("("+(parseInt(TAFrder))+")");
	}
});


function cmdSubmit_onclick(){
	if($(".CheckBoxShow:checked").length<=0){
		alert("未選擇訂單");
	}else{
		var msg="是否確定金額核對無誤？";
		if(confirm(msg)){
			$("#form1").attr("action","<?php echo GetSCRIPTNAME(); ?>?option=AgreeOrder");
			$("#form1").submit();
		}
	}	
}
	    ...
	    <td align="center">
            ...
            <table width="100%" border="1" cellspacing="0" bordercolorlight="#666666" bordercolordark="#FFFFFF">
                <tr>
                    ...
                    <td width="5%" align="right" bgcolor="#EEEEEE">
						<input type="button" value="核帳完成" id="cmdDel" name="cmdDel" onClick="cmdSubmit_onclick('<?php echo GetSCRIPTNAME(); ?>');" >
						<?php	if($AdminView){	?>
                    ...
                    <td bgcolor="#A3BFE2">
                        <table cellspacing="1" cellpadding="2" width="100%" border="0">
	                        <tr style="font-size:13;color:#E0EFF8" bgcolor="#005783" align="center">
								...
								<td nowrap width="80" style="font-size:13px;"><font color="#FFFFFF">帳號後五碼</font></td>
								<td nowrap width="80" style="font-size:13px;"><font color="#FFFFFF">付款方式</font></td>
								<td nowrap width="80" style="font-size:13px;"><font color="#FFFFFF">金額</font></td>
								...
								<td nowrap width="80" style="font-size:13px;"><font color="#FFFFFF">購買方式</font></td>
							</tr>
                        <?php
