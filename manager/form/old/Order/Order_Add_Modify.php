...
<?php
	...
	if($Option=="Add"){
		...
		$A="'".$G[0]."','".$Temp_Order_List["MemberID"]."','".Date('Y-m-d H:i:s')."','".$Temp_Order_Record["PayHandle"]."','訂單取消','是'";
		$SQL = "Insert Into orderrecord(".$Q.") values(".$A.")";
		$Rs = $Conn->prepare($SQL);
		$Rs->execute();
		ReturnToPage($MainFileName,"訂單已取消","");
	}elseif($Option=="AgreeOrder"){
		$MFun="AddShow";
		$SFun="AddScript";
		$SN=$G[0];
		$Param=array(
...
function cmdSubmit_onclick(){
	var msg="是否確定金額核對無誤？";
	if(confirm(msg)){
		$("#form1").attr("action","<?php echo GetSCRIPTNAME(); ?>?option=AgreeOrder&G0=<?php  echo $G[0] ?>");
		$("#form1").submit();
	}
}
</script>
...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	...
	<tr>
		<td align="center">
			...
			<input type="button" value="核帳完成" id="cmdF" name="cmdF" onClick="cmdSubmit_onclick();">
			<input type="button" value="取消訂單" id="cmdC" name="cmdC" onclick="cmdCancelCop_onclick();">
