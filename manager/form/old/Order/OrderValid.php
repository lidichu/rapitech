...
<?php
	...
	//參數設定
		...
		$TableTitle = $Title04;
		//要查詢的欄位
		$SQLFields="OL.SerialNo,OL.OrderNumber,date_format(OL.OrderDate,'%Y-%m-%d'),OL.PayMethod,OL.TotalPrice,OL.MemberName,OL.MemberID,date_format(OL.ShipDate,'%Y-%m-%d'),OL.BuyWay";
		//預設排序方式
		...
		//修改資料網頁名稱
		$ModifyFileName = $ModifyFileName04;		
		//按鈕式超連結,其變數值為要聯結的網頁名稱
		$DBTableName[0]= "";
		//修改資料庫名稱
		$DBTable = $DatabaseName04;
		//顯示資料庫名稱
		$DBTable_S = $DatabaseName04_S;
		//可查詢欄位
		$QueryField["OrderNumber"]="訂單編號";
		$QueryField["MemberID"]="會員編號";
		$QueryField["MemberName"]="會員姓名";
		$QueryField["BuyWay"]="購買方式";
		//來源超連結，需傳送到超連結
		$QueryList=Array();
		// 查詢參數
		$QueryParame = array();
		...
		//PageCount處理
		$PageCount = 0;
		$Query.=" and OD.IsNew='是' and OD.ManagerHandle ='交易完成' and OL.SerialNo = OD.G0";
