...
<?php
	...
	//參數設定
		...
		$TableTitle = $Title03;
		//要查詢的欄位
		$SQLFields="OL.SerialNo,OL.OrderNumber,date_format(OL.OrderDate,'%Y-%m-%d'),OL.PayMethod,OL.TotalPrice,OL.MemberName,OL.MemberID,OL.ShipDate,OL.BuyWay";
		//預設排序方式
		...
		//修改資料網頁名稱
		$ModifyFileName = $ModifyFileName03;		
		//按鈕式超連結,其變數值為要聯結的網頁名稱
		$DBTableName[0]= "";
		//修改資料庫名稱
		$DBTable = $DatabaseName03;
		//顯示資料庫名稱
		$DBTable_S = $DatabaseName03_S;
		//可查詢欄位
		...
		//PageCount處理
		$PageCount = 0;
		$Query.=" and OD.IsNew='是' and OD.ManagerHandle ='已出貨' and OL.SerialNo = OD.G0";
...
function cmdSubmit_onclick(){
	if($(".CheckBoxShow:checked").length<=0){
		...
		var msg="是否確定交易完成？";
		if(confirm(msg)){
			$("#form1").attr("action","<?php echo GetSCRIPTNAME(); ?>?option=OrderValid");
			$("#form1").submit();
		}
	}	
}
function cmdCancel_onclick(){
	if($(".CheckBoxShow:checked").length<=0){
		...
		var msg="是否確定交易取消？";
		if(confirm(msg)){
			$("#form1").attr("action","<?php echo GetSCRIPTNAME(); ?>?option=OrderInvalid");
			$("#form1").submit();
		}
	}	
}
                    ...
                    <td bgcolor="#A3BFE2">
                        <table cellspacing="1" cellpadding="2" width="100%" border="0">
	                        <tr style="font-size:13;color:#E0EFF8" bgcolor="#005783" align="center">
								...
								<td nowrap width="80" style="font-size:13px;"><font color="#FFFFFF">出貨日期</font></td>
