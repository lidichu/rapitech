(function($) {
    ...
    $.fn.twzipcode = function(settings) {
        ...
        return this.each(function() {
			...
			var area_obj = $('#' + o.area);
			var zipcode_obj = null;
			if(_settings.zipcode!=""){
				zipcode_obj = $('#' + o.zipcode);
			}
			var p_selected, str;
			if(county_obj.length > 0){
				county_obj.css({width:'90px'});
				...
				county_obj.bind('change', function(){
					if(zipcode_obj!=null) zipcode_obj.val('');
    ...
    $.fn.twzipcode.createArea = function(county_obj, area_obj, zipcode_obj, countySel, areaSel){
		...
		if(county_obj.length > 0){
			if(area_obj.length > 0){
				...
				if(county_obj.find(':eq(0)').prop('selected') == true){
				...
				if(county_obj.find(':eq(0)').prop('selected')){
					area_obj.find(':eq(0)').prop('selected', true);
				}
				if(zipcode_obj!=null){
