...
jQuery.fn.Carousel01 = function(settings){
	...
	return this.each(function(objindex) {
		...
		for(i=0;i<_settings.DivIDs.length;i++){
			if(i==0){
				$Item = $("<div><img src='"+_settings.FocusPIC+"' border='0' /></div>");
			}else{
				$Item = $("<div><img src='"+_settings.UnFocusPIC+"' border='0' /></div>");
			}			
		...
		//設定圖片按鈕列位置
		$ItemBar.css({top:$Main.height()-$ItemBar.height()-5,left:parseInt(($Main.width()-18*ItemArray.length)/2,10)});
		//建立計時器
		var Timer = 0;
		//目前播放的圖片索引
		var PicTop = 0;
		...
		var EffectAction = function(CurTop){
			ItemArray[PicTop].find("img").attr("src",_settings.UnFocusPIC);
			PicTop = CurTop;
			ItemArray[PicTop].find("img").attr("src",_settings.FocusPIC);
			$PicDown.html($("#"+_settings.DivIDs[CurTop]).html());
			StopAll();
			$PicUp.css({"opacity":1}).animate({
					...
					complete: function() {
					if(autoPlay && _settings.DivIDs.length > 1){
			...
			$PicDown.css({"opacity":0}).animate({
					...
					duration:  _settings.EffectTime,
					complete: function() {
					$PicUp.html($("#"+_settings.DivIDs[CurTop]).html());
				}
			});
		}
		//設置Item Click 事件
		...
		if(ItemArray.length > 0){
			$PicUp.html($("#"+_settings.DivIDs[PicTop]).html()).show();
