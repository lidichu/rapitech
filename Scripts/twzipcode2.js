(function( $ ){
	$.fn.TwZipCode = function(_settings){
		return this.each(function(i){
			var settings = {
				CountryFieldName : 'Country',
				AreaFieldName:'Area',
				ZipCodeFieldName:'ZipCode',
				CountryDefaultValue : '縣/市',
				AreaDefaultValue: '鄉/鎮/市/區',
				CurrentCountryValue:'',
				CurrentAreaValue:'',
				CurrentZipCodeValue:''
			}
			...
			//產生縣市物件
			var Country = $("#" + settings.CountryFieldName);
			//產生分區物件
			var Area = $("#" + settings.AreaFieldName);
			//若ZipCodeFieldName不為空，則產生 郵遞區號物件
			var ZipCode = null;
			
			
			if(settings.ZipCodeFieldName!=""){
				ZipCode = $("#" + settings.ZipCodeFieldName);
			}
			
			
			//初始化縣市物件
			CityOptionHtml = '<option value="">' + settings.CountryDefaultValue + '</option>';
			for (var CityName in $.fn.TwZipCode.ZipCode){
				if(CityName!=settings.CurrentCountryValue){
					CityOptionHtml += '<option value="' + CityName + '">' + CityName + '</option>';
				}else{
					CityOptionHtml += '<option value="' + CityName + '" selected="selected">' + CityName + '</option>';
				}
			}
			Country.html(CityOptionHtml);
			
			
			//增加縣市物件Change事件處理
			Country.bind("change",function(){
				AreaOptionHtml = '<option value="">' + settings.AreaDefaultValue + '</option>';
				for (var AreaName in $.fn.TwZipCode.ZipCode[$(this).val()]){
					AreaOptionHtml += '<option value="' + AreaName + '">' + AreaName + '</option>';
				}
				Area.html(AreaOptionHtml);
				Area.change();
			});
			...
			if(settings.CurrentAreaValue!=""){
				for (var AreaName in $.fn.TwZipCode.ZipCode[settings.CurrentCountryValue]){
					if(AreaName==settings.CurrentAreaValue){
						AreaOptionHtml += '<option value="' + AreaName + '" selected="selected">' + AreaName + '</option>';
					}else{
						AreaOptionHtml += '<option value="' + AreaName + '">' + AreaName + '</option>';
					}
				}
			...
			if(ZipCode){
				Area.bind("change",function(){
					if(Country.val() != ""){
						if($.fn.TwZipCode.ZipCode[Country.val()][Area.val()]){
							ZipCode.val($.fn.TwZipCode.ZipCode[Country.val()][Area.val()]);
						}else{
							ZipCode.val("");
						}
					}else{
				...
				if(settings.CurrentCountryValue!="" && settings.CurrentAreaValue!=""){
					ZipCode.val($.fn.TwZipCode.ZipCode[settings.CurrentCountryValue][settings.CurrentAreaValue]);
				}
				if(settings.CurrentZipCodeValue!=""){
					ZipCode.val(settings.CurrentZipCodeValue);
				}
			}
		});
	}
